...
const baseTypeIndexModule = require('./template/typeIndexModule');
const { GET_ALL_MESSAGE_REG, GET_FUNC_MESSAGE_REG, BASE_TYPE, PROTO_FILE_PATH, INGORE_FILE } = require('./resource');
...
async function checkWeakImport(filePath) {
    return new Promise(resolve => {
        fs.readFile(filePath, async (__err, originCode) => {
            const code = iconv.decode(originCode, 'utf-8');
            const reg = new RegExp(`import (${INGORE_FILE.map(item => `"${item}"`).join('|')})`, 'g');

            if (reg.test(code)) {
                const changeWeakCode = code.replace(reg, `import weak $1`);
                fs.writeFileSync(filePath, changeWeakCode, {
                    encoding: 'utf-8',
                });
            }
            resolve();
...
async function batchChangeWeakImport(list) {
    await list.reduce(async (_last, next) => {
        return await checkWeakImport(next);
    }, readSignFile(list[0]));
    return true;
}
    ...
    if (name.indexOf('.') > 0) {
        ...
        for (let i = 0; i < cacheList.length; i++) {
            ...
            if (parent) {
                ...
                if (parentMessage && parentMessage.nested && parentMessage.nested[name]) {
                    // 如果有values, 则取values(枚举), 否则去查内嵌消息
                    message = parentMessage.nested[name].values || parentMessage.nested[name];
                    break;
                }
            }
        }
...
function writeTypeIndexFile(list) {
    return new Promise(async resolve => {
        const fileName = `./type/index.ts`;
        const filePath = path.resolve(__dirname, fileName);
        await write(filePath, baseTypeIndexModule(list));
        resolve();
    });
}
...
getAllFilePath()
    ...
    .then(() => {
        const filePath = path.resolve(__dirname, '../../../../../node_modules/prettier/bin-prettier.js');
        childProgress.exec(`node ${filePath} --write ./src/js/lib/protocol/protocol-auto-task`, (err, _code) => {
            if (err) {
                console.log(err);
                return;
            }
